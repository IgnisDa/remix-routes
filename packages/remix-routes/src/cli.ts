...
const DEFAULT_OUTPUT_DIR_PATH = "./app";
		...
		routes.forEach((route) => {
			...
			if (route.id === "root") {
				...
				const fullPath = currentPath.reduce(
					(acc, curr) =>
						[acc, trimSlash(curr.path)].filter((p) => p != undefined).join("/"),
					"",
				);
...
function generate(
	...
	const outputPath = path.join(remixRoot, flags.outputDirPath);
